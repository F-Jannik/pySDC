...
from pySDC.implementations.problem_classes.HeatEquation_1D_test import numpy_heat, numpy_heat_mass

from pySDC.implementations.sweeper_classes.imex_1st_order_mass import imex_1st_order_mass
from pySDC.implementations.sweeper_classes.imex_1st_order_mass_test import imex_1st_order_mass_test
...
from pySDC.implementations.sweeper_classes.imex_1st_order_test import imex_1st_order_test

from pySDC.HookClass_numpy_output_test import fenics_output
...
def run_simulation(ml=None, mass=None, dt=0.2, maxiter=20):
    ...
    description = dict()
    if mass:
        description['problem_class'] = numpy_heat_mass
        description['sweeper_class'] = imex_1st_order_mass_test
        description['base_transfer_class'] = base_transfer_mass
    else:
        description['problem_class'] = numpy_heat
        description['sweeper_class'] = imex_1st_order_test  # pass sweeper (see part B)
...
def visualize(args):
    ...
    if "sdc_noM" in args:
        errors_sdc_noM = np.load('errors_sdc_noM.npy', allow_pickle=True)
        plt_helper.newfig(240, 1, ratio=0.8)
    ...
    if "sdc_M" in args:
        errors_sdc_M = np.load('errors_sdc_M.npy', allow_pickle=True)
        plt_helper.plt.semilogy(
    ...
    if "mlsdc_noM" in args:
        errors_mlsdc_noM = np.load('errors_mlsdc_noM.npy', allow_pickle=True)
        plt_helper.plt.semilogy(
            [err[0] for err in errors_mlsdc_noM],
            [err[1] for err in errors_mlsdc_noM],
            lw=2,
    ...
    if "mlsdc_M" in args:
        errors_mlsdc_M = np.load('errors_mlsdc_M.npy', allow_pickle=True)
        plt_helper.plt.semilogy(
...
if __name__ == "__main__":
    ...
    if(True):
        with open("Iter_Errors_noM.txt", "w") as f:
            ...
            for maxiter in maxiters:
                for dt in dts:
                    ...
                    tuple = (maxiter, dt, list(errors[-1][-1]))
                    f.write(str(tuple))
                    f.write("\n")

        with open("Iter_Errors_M.txt", "w") as f:
